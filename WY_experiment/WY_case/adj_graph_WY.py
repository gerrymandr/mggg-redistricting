...
WY_network = nw.network.Network(file_dir, unique_segs=True, node_sig=None)

# get an edge list and transform to a matplotlib collection
edge_list = []
for start, end in WY_network.edges:
    try:
        edge_list.append((cents[WY_network.node_coords[start]], cents[WY_network.node_coords[end]]))
    except:
